...
import { idleManager } from '../imap/idleManager';
...
const RegisterSchema = z.object({
  ...
  imapUser: z.string().email().max(254),
  imapPass: z.string().min(1).max(256),
  pushMissedCalls: z.boolean().optional(),
});
...
router.post('/devices/register', writeLimiter, requireAuth, async (req, res) => {
  ...
  const { deviceToken, imapUser, imapPass, pushMissedCalls } = parsed.data;
  ...
  const saveResult = saveDevice(
    {
      ...
      pushMissedCalls: pushMissedCalls !== false,
    },
    imapUser
  );
  ...
  idleManager.addOrRestart(deviceToken);
...
router.delete('/devices/:token', writeLimiter, requireAuth, (req, res) => {
  ...
  if (deleted === true) {
    ...
    idleManager.removeByToken(token);
