...
export interface Device {
  ...
  imap_uid_next_at_register: number | null;
  /** Number of consecutive IMAP auth failures (reset on successful poll). */
  auth_fail_count: number;
  /** Optional deactivation reason used for operator visibility. */
  disabled_reason: string | null;
  /** If true, send push notifications for missed calls without voicemail. */
  push_missed_calls: number;
}
...
function initSchema() {
  getDb().exec(`
    ...
    CREATE TABLE IF NOT EXISTS devices (
      ...
      push_missed_calls INTEGER NOT NULL DEFAULT 1
    );
  ...
  migrateDevicesAuthFailureFields();
  migrateDevicesPushMissedCallsField();
...
function migrateDevicesAuthFailureFields(): void {
  const cols = getDb().prepare(`PRAGMA table_info(devices)`).all() as { name: string }[];
  if (!cols.some((c) => c.name === 'auth_fail_count')) {
    getDb().exec(`ALTER TABLE devices ADD COLUMN auth_fail_count INTEGER NOT NULL DEFAULT 0`);
  }
  if (!cols.some((c) => c.name === 'disabled_reason')) {
    getDb().exec(`ALTER TABLE devices ADD COLUMN disabled_reason TEXT`);
  }
}

function migrateDevicesPushMissedCallsField(): void {
  const cols = getDb().prepare(`PRAGMA table_info(devices)`).all() as { name: string }[];
  if (cols.some((c) => c.name === 'push_missed_calls')) return;
  getDb().exec(`ALTER TABLE devices ADD COLUMN push_missed_calls INTEGER NOT NULL DEFAULT 1`);
  // Roll out missed-call pushes for all existing devices by default.
  getDb().exec(`UPDATE devices SET push_missed_calls = 1`);
}
...
export function saveDevice(input: SaveDeviceInput, imapUserPlaintext: string): SaveDeviceResult {
  ...
  const pushMissedCalls = input.pushMissedCalls ? 1 : 0;
  ...
  getDb().prepare(`
    INSERT INTO devices (
      device_token, imap_user, imap_pass, imap_host, imap_port, baseline_cutoff, imap_uid_next_at_register,
      auth_fail_count, disabled_reason, push_missed_calls
    )
    VALUES (?, ?, ?, ?, ?, ?, ?, 0, NULL, ?)
    ON CONFLICT(device_token) DO UPDATE SET
      ...
      push_missed_calls = excluded.push_missed_calls,
...
export function getDeviceById(deviceId: number): Device | null {
  const row = getDb()
...
export function getActiveDeviceById(deviceId: number): Device | null {
  const row = getDb()
...
export function getDeviceByToken(deviceToken: string): Device | null {
  const row = getDb()
...
export function getActiveDeviceByToken(deviceToken: string): Device | null {
  const row = getDb()
    .prepare('SELECT * FROM devices WHERE device_token = ? AND active = 1')
    .get(deviceToken) as DeviceRow | undefined;
  return row ? (row as unknown as Device) : null;
}

/**
 * Increase consecutive auth-failure count and optionally pause device polling.
 * Returns current count and whether the device was paused by this call.
 */
export function recordDeviceAuthFailure(
  deviceId: number,
  threshold: number
): { count: number; paused: boolean } {
  const safeThreshold = Number.isFinite(threshold) && threshold > 0 ? Math.floor(threshold) : 3;
  const db = getDb();
  db.prepare(
    `UPDATE devices
     SET auth_fail_count = COALESCE(auth_fail_count, 0) + 1
     WHERE id = ?`
  ).run(deviceId);

  const row = db
    .prepare(`SELECT auth_fail_count, active FROM devices WHERE id = ?`)
    .get(deviceId) as { auth_fail_count: number; active: number } | undefined;
  if (!row) {
    return { count: 0, paused: false };
  }

  const count = Number.isFinite(row.auth_fail_count) ? row.auth_fail_count : 0;
  if (row.active === 1 && count >= safeThreshold) {
    db.prepare(
      `UPDATE devices
       SET active = 0, disabled_reason = 'invalid_login'
       WHERE id = ?`
    ).run(deviceId);
...
/** Reset auth-failure state after successful poll. */
export function clearDeviceAuthFailures(deviceId: number): void {
  getDb()
    .prepare(
      `UPDATE devices
       SET auth_fail_count = 0, disabled_reason = NULL
       WHERE id = ? AND (COALESCE(auth_fail_count, 0) > 0 OR disabled_reason IS NOT NULL)`
    )
    .run(deviceId);
}
