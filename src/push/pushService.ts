...
import { MissedCall, Voicemail } from '../imap/imapService';
...
export async function sendMissedCallPush(
  ...
  const caller = formatCallerNumber(missedCall.callerNumber || '');
  ...
  const payload: Record<string, unknown> = {
    type: 'missed_call',
    uid: missedCall.uid,
    receivedAt: missedCall.receivedAt.toISOString(),
  };
  if (PUSH_INCLUDE_CALLER) {
    payload.callerNumber = missedCall.callerNumber;
  }
  notification.payload = payload;
  ...
  try {
    const result = await getProvider().send(notification, deviceToken);
    if (result.sent && result.sent.length > 0) {
      console.log('[APNs] Missed-call push sent to ' + deviceToken.substring(0, 8) + '...');
      return { ok: true };
    }
    ...
    if (result.failed.length > 0) {
      ...
      const detail =
        ...
        (reason != null
          ? `HTTP ${f.status ?? '?'} ${reason}`
          : JSON.stringify(f));
      console.error('[APNs] Missed-call push failed:', detail);
      return { ok: false, apnsReason: reason };
    }
    ...
    console.error('[APNs] Error sending missed-call push:', formatErr(err));
    return { ok: false };
  }
}
