...
Öffentlich einsehbarer Quellcode des Backend-Dienstes, der die **Böxle-iOS-App** mit der **Telekom-SprachBox** verbindet. Veröffentlicht, damit Nutzer und Dritte **nachvollziehen können, was auf dem Server mit ihren Daten passiert**.
...
3. **Abfrage (IMAP IDLE)**
   Für jedes aktive Gerät hält der Server eine eigene IMAP-Verbindung offen (`SprachBox`) und wartet per **IDLE** auf neue Nachrichten. Bei IMAP-`EXISTS`-Signalen werden neue Metadaten sofort gelesen; bei Verbindungsproblemen erfolgen automatische Reconnects mit Backoff. Plaintext-Credentials werden nur für den Aufbau/Refresh der jeweiligen Device-Verbindung im RAM entschlüsselt, nie global gecacht.
...
| Daten | Wo | Zweck |
|-------|----|-------|
| IMAP-E-Mail / App-Passwort | HTTP-Body bei Login und Registrierung | IMAP-Validierung und späterer IDLE-Betrieb; in der DB **AES-256-GCM-verschlüsselt** |
...
- **`devices`** — APNs-Token, verschlüsselter IMAP-Nutzer/-Passwort, Zeitstempel, Baseline-UIDNEXT, Zustandsfelder (u. a. für Auth-Fehler/Deaktivierung).
...
- IMAP-Abruf und Betreff-Parsing: `src/imap/imapService.ts`, IDLE-Manager: `src/imap/idleManager.ts`, Baseline-/Seen-/Housekeeping-Logik: `src/imap/poller.ts`
